...
let menu = []
menu.push(createMenuItem('Pasta', 10, 'Italian'));
menu.push(createMenuItem('Falafel', 5, 'Egyptian'));
menu.push(createMenuItem('Fried Chicken', 8, 'American'));
...
export const burger = {
  ...
  discount: function(customerType) {
    ...
    if (customerType==="teacher") {
      discountedPrice = this.price*(1-0.25);
    } else if (customerType==="student") {
      discountedPrice = this.price*(1-0.25);
    } else {
      discountedPrice = this.price*(1-0.1);
    }
...
function getLastReview(arr) {
  ...
  let lastReviewSentence = `${lastReview.name} gave the restaurant a ${lastReview.rating} star review, and their feedback was: ${lastReview.feedback}`
  return (lastReviewSentence);
} 
